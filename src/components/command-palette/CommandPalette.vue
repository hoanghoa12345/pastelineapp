    ...
    <Dialog as="div" @close="emit('close')" class="relative z-20">
      ...
      <div class="fixed inset-0 overflow-y-auto p-4 pt-32">
        <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
          ...
          <DialogPanel
            class="w-full mx-auto max-w-xl transform rounded-xl bg-white text-left align-middle shadow-xl transition-all">
            <Combobox v-model="selected">
              <div class="relative mt-1">
                <div
                  class="relative w-full cursor-default overflow-hidden rounded-lg bg-white text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
                  <ComboboxInput
                    class="w-full border-none outline-none py-3 pl-10 pr-3 text-sm leading-5 text-gray-900 focus:ring-0"
                    @change="query = $event.target.value" placeholder="Search..." />
