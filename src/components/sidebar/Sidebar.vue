<template>
  <aside class="w-72 transition-all duration-300 h-full z-50"
    :class="clsx(state.openDrawer ? 'ml-0' : '-ml-72', isMobile && 'absolute')">
    <nav
      class="flex flex-col w-72 h-full py-4  bg-[#ebf1f4] dark:bg-dark-800 border-r border-gray-200 dark:border-gray-700">
      <ul class="flex-1">
        <div class="flex items-center justify-between mx-4">
          ...
          <div class="flex space-x-2">
            <button class="hover:text-violet-600">
              <Icon class="w-5 h-5 dark:text-white" icon="ph:bell" />
            </button>
        ...
        <div class="mx-4 my-4">
          <div class="flex items-center justify-between">
            ...
            <div>
              <button @click="state.setOpenCreate(true)"
                class="w-8 h-8 bg-violet-600 rounded-full flex justify-center items-center group">
                <Icon class="w-5 h-5 text-gray-300 group-hover:scale-110" icon="ic:baseline-plus" />
              </button>
            </div>
          </div>
        </div>
      ...
      <div class="dark:bg-dark-800 mx-4">
        <button @click="state.setOpenCreate(true)" type="button"
      ...
      <Form @submit="onSubmit" v-slot="{ errors }">
        <div>
          ...
          <Field type="text" name="title" id="title" placeholder="Enter page title" :rules="validateTitle"
            class="border sm:text-sm rounded-lg block w-full p-2.5" label="Page title" :class="clsx(errors?.title
