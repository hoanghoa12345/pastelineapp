<template>
  <!-- Sidebar -->
  <aside :class="`fixed top-0 left-0 z-10 w-64 h-screen pt-14 transition-transform ${drawer.isOpen ? 'transform-none' : '-translate-x-full'
    } bg-white border-r border-gray-200 md:translate-x-0 dark:bg-gray-800 dark:border-gray-700`" aria-label="Sidenav"
      ...
      <ul class="space-y-2">
        ...
        <Disclosure as="li" v-slot="{ open }">
          <DisclosureButton type="button"
            ...
            <svg aria-hidden="true" :class="open ? '' : '-rotate-90 transform'" class="w-6 h-6" fill="currentColor"
          ...
          <DisclosurePanel as="ul" class="py-2 space-y-2">
            ...
            <li v-else>
              <span
                class="flex items-center p-2 pl-11 w-full text-sm font-medium text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Not
                have favorite notes</span>
            </li>
          </DisclosurePanel>
        </Disclosure>
        <Disclosure as="li" v-slot="{ open }">
